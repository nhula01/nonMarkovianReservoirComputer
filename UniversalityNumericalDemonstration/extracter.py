...
def separation(file_names, Nfading = 10, Ntraining = 290, Ntotal = 500, tau = 1):
    ...
    for file_name in file_names:
        ...
        X = extract_delayed_single_column(df, 'PQ0', N=10, repeat_steps=1)
        X_features = X[0:K]
        ...
        y_output.append(predict(X_test, W))
        X_observables.append(X_features)
        
    return y_output, X_observables

def addition(file_names, Nfading = 10, Ntraining = 290, Ntotal = 500, tau = 1):
    task = mackey_glass(Npoints=Ntotal)
    Ntesting = Ntotal - Nfading - Ntraining
    K = Ntotal - tau
    y_output = []
